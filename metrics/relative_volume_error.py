...
# 结果输出文件路径
output_file = os.path.join(inference_root, "relative_volume_errors.json")

# 初始化结果字典
relative_volume_errors = {}
...
for sample_folder in os.listdir(inference_root):
    ...
    if os.path.isdir(sample_path):
        ...
        for sub_folder in os.listdir(sample_path):
            ...
            # 确保子文件夹存在并且包含 real_volumes.json 文件
            ...
            if os.path.exists(real_volume_file):
                ...
                with open(real_volume_file, 'r') as f:
                    ...
                    predicted_volume = predicted_data["resize3d.obj"]["volume"]
                ...
                with open(ground_truth_volume_file, 'r') as f:
                    ...
                    if obj_filename in ground_truth_data:
                        ground_truth_volume = ground_truth_data[obj_filename]["volume"]

                        # 计算相对体积误差
                        relative_error = abs(predicted_volume - ground_truth_volume) / ground_truth_volume

                        # 保存每个帧的结果
                        relative_volume_errors[sub_folder] = relative_error

                        # 提取样本名（如 "F0001PL7S3R4"）
                        sample_name = sub_folder.split('_')[0]

                        # 将误差加入到对应样本的列表中
                        sample_errors[sample_name].append(relative_error)
                    else:
...
for sample_name, errors in sample_errors.items():
    ...
    relative_volume_errors[sample_name] = average_error
...
if all_sample_errors:
    ...
    relative_volume_errors["overall_average_error"] = overall_average_error

# 将结果写入 JSON 文件
with open(output_file, 'w') as f:
    json.dump(relative_volume_errors, f, indent=4)

print(f"Relative volume errors written to {output_file}")
