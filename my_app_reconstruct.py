...
DEFAULT_BACKGROUND_CHOICE = "自动去背景"
...
with gr.Blocks(css=css_content, theme="default", elem_classes="gradio-app") as demo:
    ...
    with gr.Row(variant="panel", elem_classes="input-container"):
        with gr.Column(scale=3):
            ...
            examples_gallery = gr.Gallery(
                value=initial_gallery_paths, label="当前批次源图像", columns=5,
                height="auto", object_fit="contain", preview=True, interactive=False
            )
        with gr.Column(scale=2):
            gr.Markdown("### 当前处理图像预览") # Title remains static
            # Keep the static label here
            current_image_display = gr.Image(
                label="当前处理图像预览",
                value=None, # Start empty
                type="pil", image_mode="RGBA",
                height=500, interactive=False, elem_classes="display-container"
            )

    # --- Control Buttons (Moved Before Log) ---
    with gr.Row(variant="panel"):
        batch_button = gr.Button("开始批量生成", variant="primary", elem_classes="button")
        stop_button = gr.Button("中止处理", variant="stop", interactive=False, elem_classes="button")

    # --- Progress Reporting (Plain Textbox Log) ---
    with gr.Row(variant="panel"):
    ...
    with gr.Row(variant="panel", elem_classes="output-container"):
        with gr.Column(scale=1, min_width=300):
            gr.Markdown("### 视图输出")
            processed_image = gr.Image(
                 label="预处理后图像", interactive=False, type="pil",
                 image_mode="RGB", height=200, elem_classes="output-container-inner"
            )
            image_output = gr.Image(
                 interactive=False, label="多视角RGB图像",
                 height=150, elem_classes="output-container-inner"
            )
            xyz_output = gr.Image(
        ...
        with gr.Column(scale=1, min_width=300):
            ...
            output_model = gr.Model3D(
                ...
                height=450, elem_classes="model3d-output"
            )
    ...
    log_text_state = gr.State("") # Store plain log text

    # Plain text log appending
    def append_log(current_log, message):
        # Optional: Limit log length if memory becomes an issue
        # max_lines = 500
        # lines = current_log.split('\n')
        # if len(lines) > max_lines:
        #     current_log = "\n".join(lines[-max_lines:]) # Keep last N lines
        return current_log + message + "\n"

    # --- Batch Process Generator (Plain Text Log & Label Indicator) ---
    def batch_process_generator(current_start_index, all_paths, saved_zip_paths_list,
        ...
        if not all_paths:
             ...
             yield {
                ...
                saved_zip_paths_state: [], output_files_group: gr.update(visible=False),
                log_text_state: log
             }
        ...
        while current_start_index < total_images_overall and processing_flag.is_set():
            ...
            # Initial yield for batch: update gallery and log, clear previous outputs
            yield {
                examples_gallery: gr.update(value=current_batch_paths),
                progress_text: gr.update(value=log),
                current_image_display: gr.update(value=None, label="当前处理图像预览"), # Clear display and reset label
            ...
            with tempfile.TemporaryDirectory() as batch_temp_dir:
                ...
                for i, img_path in enumerate(current_batch_paths):
                    if not processing_flag.is_set():
                        ...
                        yield { progress_text: gr.update(value=log), log_text_state: log }
                        break

                    img_start_time = time.time()
                    current_img_index_overall = start_index + i
                    img_seed = seed_counter + i
                    base_name = Path(img_path).name
                    img_progress_prefix = f"{batch_info_str} ({i+1}/{current_batch_size}, {base_name})"

                    preprocessed_img_result = None
                    stage1_img_result = None
                    stage2_img_result = None
                    final_glb_path_result = None
                    original_img_pil = None # Store original image

                    try:
                        # Load original image
                        original_img_pil = Image.open(img_path).convert("RGBA")

                        # Update display to show ORIGINAL image and PROCESSING label
                        yield {
                            current_image_display: gr.update(value=original_img_pil, label=f"**⏳ 处理中: {base_name}...**")
                        }
                        ...
                        yield { progress_text: gr.update(value=log), log_text_state: log }
                        preprocessed_img_result = preprocess_image_default(img_path)

                        log = append_log(log, f"{img_progress_prefix}: 2/4 生成模型...")
                        yield { progress_text: gr.update(value=log), log_text_state: log }
                        stage1_img_result, stage2_img_result, temp_glb_path_orig, temp_obj_path_orig = gen_image_default(preprocessed_img_result, img_seed)
                        ...
                        final_glb_path_result = str(batch_temp_glb_path)
                        # --- Processing Steps Done ---

                        img_end_time = time.time()
                        log = append_log(log, f"{img_progress_prefix}: ✔️ 完成 [{(img_end_time-img_start_time):.1f}s]")
                        # Yield all results together, resetting preview label
                        yield {
                            processed_image: gr.update(value=preprocessed_img_result),
                            image_output: gr.update(value=stage1_img_result),
                            xyz_output: gr.update(value=stage2_img_result),
                            output_model: gr.update(value=final_glb_path_result),
                            progress_text: gr.update(value=log),
                            log_text_state: log,
                            current_image_display: gr.update(label="当前处理图像预览") # Reset label only
                        }
                        ...
                        # Yield updates to clear outputs, update log, reset label
                        yield {
                            progress_text: gr.update(value=log), log_text_state: log,
                            processed_image: gr.update(value=None), image_output: gr.update(value=None),
                            xyz_output: gr.update(value=None), output_model: gr.update(value=None),
                            # Keep original preview displayed, just reset label
                            current_image_display: gr.update(label="当前处理图像预览")
                        }
                        continue
                # --- End of Image Loop ---

                # --- Zip File Handling ---
                # ... (No changes needed in zipping/saving logic) ...
                if batch_results_paths:
                    ...
                    with zipfile.ZipFile(batch_zip_path_temp, 'w') as zipf:
                        for glb, obj in batch_results_paths: zipf.write(glb, Path(glb).name)
                    ...
                    yield {
                        ...
                        progress_text: gr.update(value=log),
                        saved_zip_paths_state: current_run_saved_zips,
                        output_files_group: gr.update(visible=True),
                        log_text_state: log
                    }
                else:
                    batch_end_time = time.time()
        ...
        yield {
            ...
            saved_zip_paths_state: current_run_saved_zips,
            output_files_group: gr.update(visible=bool(current_run_saved_zips)),
            log_text_state: log # Final state update
        }

    # --- Control Functions (Plain Text Log) ---
    def start_batch_processing():
        ...
        return {
            ...
            log_text_state: initial_log, # Reset state
            progress_text: gr.update(value=initial_log), # Reset display
            batch_output: gr.update(value=[], interactive=False),
            output_files_group: gr.update(visible=False),
            batch_button: gr.update(interactive=False),
            stop_button: gr.update(interactive=True),
            current_image_display: gr.update(value=None, label="当前处理图像预览") # Clear preview
        }
    ...
    def stop_processing_action(current_log_text):
        ...
        return {
             progress_text: gr.update(value=log), # Update log display
             log_text_state: log, # Update state
    ...
    def finish_processing_updates(current_log_text):
        ...
        return {
            ...
            progress_text: gr.update(value=log), # Update log display
            log_text_state: log, # Update state
            # Reset image preview label one last time if needed
            current_image_display: gr.update(label="当前处理图像预览")
        }


    # --- Event Listeners (Plain Text Log State) ---
    batch_process_event = batch_button.click(
        fn=start_batch_processing,
        outputs=[current_image_index_state, saved_zip_paths_state, log_text_state,
                 progress_text, batch_output, output_files_group,
                 batch_button, stop_button, current_image_display]
    ).then(
        fn=batch_process_generator,
        inputs=[ # Pass log text state
            current_image_index_state, all_image_paths_state, saved_zip_paths_state,
            log_text_state, # <-- Use state here
        ],
        outputs=[ # Update log text display and log text state
            examples_gallery, progress_text, # Update log text display
            batch_output, current_image_display, processed_image,
            image_output, xyz_output, output_model,
            saved_zip_paths_state,
            output_files_group,
            log_text_state # <-- Update state here
        ]
    ).then(
       fn=finish_processing_updates,
       inputs=[log_text_state], # Pass final state
       outputs=[batch_button, stop_button, progress_text, log_text_state, current_image_display] # Also reset label here
    )

    stop_button.click(
        fn=stop_processing_action,
        inputs=[log_text_state], # Pass state
        outputs=[progress_text, log_text_state, stop_button], # Update log display, state, button
