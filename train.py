...
def train(config, unk):
    ...
    if config.get("resume", False):
        ...
        if len(ckpts) == 0:
            ...
            print("loading ckpt from ", osp.join(config.ckpt_root, latest_ckpt))
    ...
    while True:
        ...
        if total_step > save_step and accelerator.is_main_process:
            ...
            torch.save(
                model.state_dict(),  # 保存整个模型的 state_dict
                osp.join(config.ckpt_root, f"model-{total_step}.pth")
            )
            save_step += config.save_interval
            logging.info("save done")

        is_eval = True
        if total_step > eval_step and is_eval:
            logging.info(f"total_step: {total_step}, eval_step: {eval_step} evaluating...")
            unet.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="configs/nf7_v3_SNR_rd_size_stroke_train.yaml"
    )
    parser.add_argument(
        "--logdir", type=str, default="/workspace/CRM/train_logs", help="the dir to put logs"
    )
    parser.add_argument(
